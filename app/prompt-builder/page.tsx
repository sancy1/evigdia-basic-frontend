...
import { Copy, Download, FileText, Code, Loader2, Plus, X, Bot, Sparkles, Shield, CheckCircle } from "lucide-react"
import { toast } from "@/hooks/use-toast"
import { cn } from "@/lib/utils"
import { AppSidebar } from "@/components/app-sidebar"
import { AppHeader } from "@/components/app-header"
                    ...
                    {generatedCode ? (
                      <div className="space-y-4">
                        ...
                        <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
                          <h3 className="text-sm font-medium mb-2 text-purple-700 dark:text-purple-300">App Summary</h3>
                          <p className="text-sm text-gray-700 dark:text-gray-300">
                            <strong>{formData.appName}</strong> - {formData.summary.substring(0, 150)}
                        ...
                        <div className="p-4 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                          ...
                          <p className="text-sm text-gray-700 dark:text-gray-300">
                            I've generated a {formData.platform || "web"} application using{" "}
                            {formData.techStack || "modern web technologies"}. The code follows{" "}
                            {formData.professionalImplementation ? "professional standards" : "standard practices"}
                            {formData.avoidErrors ? " with robust error handling" : ""}
                            {formData.avoidBreakingCode ? " while preserving existing code compatibility" : ""}.
                          </p>
                        </div>
...
│   │   └── feature-card.tsx  <-- Custom feature display component
│   └── lib/
...
└── README.md                 <-- Project documentation`}
                        ...
                        <div className="flex flex-wrap gap-2 mt-4">
                          {formData.professionalImplementation && (
                            <Badge variant="secondary" className="bg-green-100 text-green-700">
                              <CheckCircle className="h-3 w-3 mr-1" />
                              Professional Code
                            </Badge>
                          )}
                          {formData.avoidErrors && (
